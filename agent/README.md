...
## HTTP Adapter (local chat integration)

This repository now contains a small FastAPI adapter exposing HTTP endpoints suitable for integration with the frontend chat UI:

- `POST /api/v1/adk/chat` — JSON `{ message, session_id? }` returns `{ answer, session_id, steps?, retrieval_sources? }`.
- `POST /api/v1/adk/chat/stream` — streaming newline-delimited JSON chunks with `{ content }` parts and a final `{ done: true }` marker.
- `POST /api/v1/agent-chat` and `POST /api/v1/agent-chat/stream` — non-ADK-style equivalents (reuse same agent implementation).

To run the adapter locally with Docker (build from repository root `agent` folder):

```bash
docker build -t adk-agent .
docker run -p 10000:10000 --env-file .env adk-agent
```

By default the adapter listens on port `10000` inside the container.

Local development
-----------------

To run the HTTP adapter directly (without Docker) you need to install runtime dependencies locally:
...
This will run the small FastAPI adapter on port 10000 by default. If `a2a` (full ADK stack) is installed, `start_a2a.py` will prefer running the full A2A server instead.
