...
try:
    ...
    # a2a imports may not be present in local dev; import lazily
    ...
    print('Warning: a2a libraries not available locally; falling back to HTTP adapter. ', e)
...
def main():
    try:
        ...
        if _A2A_AVAILABLE:
            ...
            # Local dev fallback: run the small HTTP adapter (FastAPI) we include in this repo.
            try:
                import uvicorn
            except Exception:
                logger.error('uvicorn is not installed. To run the local adapter, install dependencies: pip install -r requirements.txt')
                raise
            print('Starting local FastAPI adapter (adk_api) on port', os.getenv('PORT', '10000'))
            uvicorn.run('src.adk_api:app', host='0.0.0.0', port=int(os.getenv('PORT', 10000)), log_level='info')
